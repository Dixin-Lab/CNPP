...
class CoupledEmbedding(nn.Module):
    ...
    def forward(self, process_idx, event_types: torch.Tensor, is_param=False):
        ...
        """
        event_types_onehot = F.one_hot(
            event_types, num_classes=self.num_types[process_idx]+1)  # [batch size, seq length, total_num]
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        # copuled embedding layer
        self.event_emb = CoupledEmbedding(
            num_types, d_model, n_iter=100, P_prior=P_prior, is_param=is_param)
