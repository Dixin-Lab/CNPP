...
def predict_test(model, process_idx, seq_times, seq_types, seq_lengths, pad, device='cpu',
    ...
    for index_ in index_range_:
        _seq_data = (seq_times[index_],
                     ...
                     seq_lengths[index_])
        if rnn:
            est, real_dt, err, real_type, est_type = model.read_predict(process_idx, *_seq_data, hmax)
        else:
            est, real_dt, err, real_type, est_type = model.read_predict(process_idx, *_seq_data, pad, device, hmax)
