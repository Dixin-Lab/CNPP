...
def get_prior_P(path0, path1, epsilon=0.0001, numItermax=5000,max_time=0.0, num_seq=2000,index=1):
    ...
    P = ot.sinkhorn(distribute_a, distribute_b, cost, epsilon,numItermax=numItermax)

    np.savez("P_" + str(len(lambda_1)) + "_prior_" + str(epsilon) +"_"+str(numItermax)+"_"+str(num_seq)+"_"+str(max_time)+"_idx_"+str(index)+".npz", P=P)
    print("P", sum(sum(P)))
...
def get_tpp_statistics(path="./exp_5313_5120_10000_0.05.pkl",num_seq=10000,max_time=0.05,index=0):
    ...
    # print("cost", cost)
    np.savez("lambda1_lambda2_const_" + str(len(lambda_1)) + "_" + str(num_seq) + "_"+str(max_time) + "_idx"+str(index)+".npz", lambda_1=lambda_1, lambda_2=lambda_2, cost=cost)
...
if __name__=="__main__":
    ...
    path = ("./Tpp_data/Syn_data/unweighted/exp/exp_50_50_2000_6_idx",6
    ...
    for idx in range(5):
        ...
        max_time=path[1]
        get_tpp_statistics(path=plk_path, num_seq=10000, max_time=max_time,index=idx)

        abpath=path[2]+"idx"+str(idx)+".npz"

        get_prior_P(path0=plk_path,path1=abpath
                    ,epsilon=0.0001,numItermax=10000
                    ,max_time=max_time,num_seq=10000,index=idx)


        Ppath=path[3]+str(idx)+".npz"
        prior_accuracy(path0=plk_path
                       ,path1=Ppath)
