...
def main():
    ...
    parser.add_argument('-plk_path', type=str, default='exp_10_10_2000_30_idx')
    parser.add_argument('-cost_path', type=str, default='lambda1_lambda2_const_10_2000_15_idx0.npz',
    ...
    parser.add_argument('-epsilon', type=int, default=0.0001, help='epsilon for sinkhorn algorithm')
    ...
    get_tpp_statistics(path=opt.plk_path, num_seq=opt.num_seq,
                       max_time=opt.max_time, index=opt.index)
    get_prior_P(path0=opt.plk_path, path1=opt.cost_path, epsilon=opt.epsilon,
                numItermax=opt.numItermax, max_time=opt.max_time, num_seq=opt.num_seq
                , index=opt.index)
