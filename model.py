...
class CoupledEmbedding(nn.Module):
    def __init__(
            self,
            num_types: List, dim: int, n_iter: int = 20):
        super().__init__()
...
class Encoder2(nn.Module):
    ...
    def __init__(
            self,
            num_types: List, d_model: int, d_inner: int,
            n_layers: int, n_head: int, d_k: int, d_v: int, dropout: float):
        super().__init__()
        ...
        # event type embedding
        self.event_emb = CoupledEmbedding(num_types, d_model, n_iter=100)
